...
POST_CROP_MP4 = True
# Crop pixels from left and right after encoding.
CROP_PX_LEFT = 50
CROP_PX_RIGHT = 50
...
fig.subplots_adjust(left=0.03, right=0.99, bottom=0.02, top=0.96)
ax.set_position([0.05, 0.02, 0.94, 0.92])
...
max_tick_xy = 5.0 * np.floor(lim_xy / 5.0)
xy_ticks = np.arange(-max_tick_xy, max_tick_xy + 0.1, 5.0)
...
ax.tick_params(colors="white", labelsize=12.5)
...
ax.xaxis._axinfo["grid"]["color"] = (1.0, 1.0, 1.0, 0.25)
ax.yaxis._axinfo["grid"]["color"] = (1.0, 1.0, 1.0, 0.25)
ax.zaxis._axinfo["grid"]["color"] = (1.0, 1.0, 1.0, 0.25)
ax.xaxis._axinfo["axisline"]["color"] = (1.0, 1.0, 1.0, 1.0)
ax.yaxis._axinfo["axisline"]["color"] = (1.0, 1.0, 1.0, 1.0)
ax.zaxis._axinfo["axisline"]["color"] = (1.0, 1.0, 1.0, 1.0)
...
# Trails
trail_lines = []
trail_colors = plt.cm.tab10(np.linspace(0, 1, N_STARS))
...
time_text = ax.text2D(0.02, 0.95, "", transform=ax.transAxes, fontsize=16.8)
stage_text = ax.text2D(0.02, 0.90, "", transform=ax.transAxes, fontsize=16.8)
...
if SAVE_MP4:
    ...
    if POST_CROP_MP4:
        cropped_mp4 = MP4_NAME.replace(".mp4", "_cropped.mp4")
        crop_filter = f"crop=iw-{CROP_PX_LEFT + CROP_PX_RIGHT}:ih:{CROP_PX_LEFT}:0"
        ffmpeg_cmd = [
            "ffmpeg",
            "-y",
            "-i",
        ...
        try:
            ...
            print(f"Saved cropped MP4: {cropped_mp4}")
        except FileNotFoundError:
            print("ffmpeg not found: skipping post-encode crop.")
        except subprocess.CalledProcessError as e:
            print("ffmpeg crop failed; keeping original MP4.")
            print(e.stderr)
