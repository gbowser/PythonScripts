...
def compute_state(t):
    ...
    Z = A * np.cos(phi_snail)                      # internal angle-like coord [rad]
    Vz = A * np.sin(phi_snail)                     # internal velocity-like coord [rad/s]
...
for j, idx in enumerate(sample_indices):
    # physical pendulum angle θ(t) for each sampled pendulum
    theta_t = A[idx] * np.sin(Omega_pend[idx] * t_array)  # radians
    theta_tracks_deg[j] = np.degrees(theta_t)             # convert to degrees

# --------------------------------------------------
# Figure and axes
# --------------------------------------------------
fig, (ax_pend, ax_time, ax_snail) = plt.subplots(1, 3, figsize=(15, 5))
plt.rcParams["axes.labelsize"] = 34   # pick a size you like (e.g. 16, 18, 20)
...
# --- TITLE CHANGE: V_theta with subscript ---
...
# --------------------------------------------------
# Linear spiral fit (Archimedean) – still fit in (Z,Vz), then convert to degrees
# --------------------------------------------------
...
anim = FuncAnimation(
    ...
    init_func=init,
    blit=False,
    # --- SPEED UP ANIMATION: halve interval ---
    interval=1000 * dt / 2,
)
