...
def phi_physical_kms2(R_kpc, z_kpc, phi_rad):
    return evaluatePotentials(mw_nonaxisymmetric, R_kpc / ro, z_kpc / ro, phi=phi_rad) * vo**2


def fz_physical_kms2_per_kpc(R_kpc, z_kpc, phi_rad):
    return evaluatezforces(mw_nonaxisymmetric, R_kpc / ro, z_kpc / ro, phi=phi_rad) * vo**2 / ro
...
for radius_kpc, color in zip(radii_kpc, colors):
    ...
    force_curve = np.array(
        [fz_physical_kms2_per_kpc(radius_kpc, z_val, phi_slice_rad) for z_val in z_force_kpc]
    )
    force_harm = -nu_squared * z_force_kpc

    omega_curve = np.array(
        [vertical_frequency_kmskpc(radius_kpc, zmax_kpc, phi_slice_rad) for zmax_kpc in zmax_grid_kpc]
    )
    omega_harm = np.full_like(zmax_grid_kpc, nu_kmskpc)

    label = rf"R = {radius_kpc:.0f} kpc"
    ax1.plot(z_plot_kpc, delta_phi_curve_thousands, color=color, linewidth=2.3, label=f"{label} true")
    ax1.plot(z_plot_kpc, delta_phi_harm_thousands, color=color, linewidth=1.8, linestyle="--", label=f"{label} harmonic")

    ax2.plot(z_force_kpc, force_curve / 1000.0, color=color, linewidth=2.3, label=f"{label} true")
    ax2.plot(z_force_kpc, force_harm / 1000.0, color=color, linewidth=1.8, linestyle="--", label=f"{label} harmonic")

    ax3.plot(zmax_grid_kpc, omega_curve, color=color, linewidth=2.3, label=f"{label} true")
    ax3.plot(zmax_grid_kpc, omega_harm, color=color, linewidth=1.8, linestyle="--", label=f"{label} harmonic")
...
for radius_kpc, color in zip(radii_kpc, colors):
    ...
    omega_full_curve = np.array(
        [
            vertical_frequency_kmskpc(radius_kpc, zmax_panel4_kpc, barphi_rad + phi_offset_rad)
...
ax4.set_title(r"$\Delta \Omega_z$ at $z_{\max}=500$ pc vs Angle", fontsize=TITLE_FONTSIZE)
...
fig_radius, (ax_radius, ax_frequency_radius) = plt.subplots(2, 1, figsize=(9, 12), sharex=True)
...
fig_radius.savefig(output_dir / f"{output_base}_radius_force_change.png", dpi=250, bbox_inches="tight")
fig_radius.savefig(output_dir / f"{output_base}_radius_force_change.pdf", bbox_inches="tight")
