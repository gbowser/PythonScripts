...
# Time of second perturbation (in displayed time units)
t_second_kick = 0.4

# Strength of the second perturbation relative to the first
kick_factor = 1.0  # >1 makes the 2nd kick stronger
...
def compute_state(t):
    """
    Compute pendulum angles + velocities and snail coordinates.

    Second perturbation:
    - For t < t_second_kick: standard SHM with amplitude A
    - For t >= t_second_kick: add a second oscillatory term (scaled by kick_factor)
      starting at t_second_kick, mimicking an impulsive "kick" to each pendulum.
    """
    # Base motion from t = 0
    phi_pend = Omega_pend * t
    theta_base = A * np.sin(phi_pend)
    theta_dot_base = A * Omega_pend * np.cos(phi_pend)

    if t < t_second_kick:
        theta = theta_base
        theta_dot = theta_dot_base
    else:
        # Additional oscillation starting at t_second_kick (second perturbation)
        phi_kick = Omega_pend * (t - t_second_kick)
        theta_kick = kick_factor * A * np.sin(phi_kick)
        theta_dot_kick = kick_factor * A * Omega_pend * np.cos(phi_kick)

        theta = theta_base + theta_kick
        theta_dot = theta_dot_base + theta_dot_kick

    # Snail coordinates unchanged
...
TITLE_Y = 1.02
ax_pend.set_title("Pendulums - Varying Lengths (2nd kick at t=0.40)", y=TITLE_Y)
...
# Snapshot times: 0.5, 1.0, 1.5 (t = 0 omitted)
