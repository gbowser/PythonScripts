...
# Increase plot-title and axis-label font sizes by 50%.
FONT_SCALE = 1.5
...
Phi_total_dimless = (
    Phi_axisym_dimless + Phi_baronly_dimless + Phi_spiralonly_dimless
)

# Optional closure check against direct total evaluation.
...
ax.set_zlabel(r"$\Phi$ [10$^3$ (km/s)$^2$]", fontsize=AXIS_LABEL_FONTSIZE)
ax.set_title(
    "Total Potential (Disk+Bar+Spiral)",
...
fig.colorbar(im, ax=ax2, shrink=0.85, pad=0.02, label=r"$\Phi$ [10$^3$ (km/s)$^2$]")
...
ax3.set_title(r"Bar Contribution $\Delta\Phi_{\mathrm{bar}}$", fontsize=TITLE_FONTSIZE)
fig.colorbar(
...
ax4.set_title(
    r"Spiral Contribution $\Delta\Phi_{\mathrm{spiral}}$", fontsize=TITLE_FONTSIZE
)
fig.colorbar(
