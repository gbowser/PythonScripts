...
The top-left panel shows an R-z meridional slice at fixed azimuth, and the other panels show x-y maps at fixed z.
...
Phi_total_dimless = (
    Phi_axisym_display_dimless
    + Phi_bar_display_dimless
    + Phi_spiral_display_dimless
)
...
print(f"axisym zero shift     = {axisym_offset_dimless * vo**2 / 1000.0:.6g} [10^3 (km/s)^2]")
print(f"bar zero shift        = {bar_offset_dimless * vo**2 / 1000.0:.6g} [10^3 (km/s)^2]")
print(f"spiral zero shift     = {spiral_offset_dimless * vo**2 / 1000.0:.6g} [10^3 (km/s)^2]")
...
# Top-left R-z meridional slice plus x-y maps
fig = plt.figure(figsize=(14, 10))
ax = fig.add_subplot(2, 2, 1)
total_display_offset_dimless = (
    axisym_offset_dimless + bar_offset_dimless + spiral_offset_dimless
)
R_slice_kpc = np.linspace(0.0, 12.0, 180)
z_slice_kpc = np.linspace(0.0, 1.5, 140)
R_slice_grid_kpc, Z_slice_grid_kpc = np.meshgrid(R_slice_kpc, z_slice_kpc)
R_slice_eval_kpc = np.clip(R_slice_grid_kpc, 0.05, None)
phi_slice_rad = 0.0
Phi_rz_dimless = np.array(
    [
        evaluatePotentials(mw_nonaxisymmetric, r / ro, z / ro, phi=phi_slice_rad)
        for r, z in zip(R_slice_eval_kpc.ravel(), Z_slice_grid_kpc.ravel())
    ]
).reshape(R_slice_grid_kpc.shape)
Phi_rz_kms2_thousands = (
    ((Phi_rz_dimless - total_display_offset_dimless) * vo**2) / 1000.0
)
Phi_rz_masked = np.ma.masked_invalid(Phi_rz_kms2_thousands)
phi_rz_finite = Phi_rz_masked.compressed()
levels_rz = np.arange(np.floor(np.min(phi_rz_finite)), np.ceil(np.max(phi_rz_finite)) + 1.0, 1.0)
im_rz = ax.contourf(
    R_slice_grid_kpc,
    Z_slice_grid_kpc * 1000.0,
    Phi_rz_masked,
    levels=levels_rz,
    cmap="viridis",
)
...
ax.set_xlabel("R [kpc]", fontsize=AXIS_LABEL_FONTSIZE)
ax.set_ylabel("z [pc]", fontsize=AXIS_LABEL_FONTSIZE)
ax.tick_params(axis="both", labelsize=TICK_LABEL_FONTSIZE)
ax.set_title(
    "Potential by Radius and Height",
    fontsize=TITLE_FONTSIZE,
)
cbar = fig.colorbar(
    im_rz, ax=ax, shrink=0.85, pad=0.02, label=r"$\Phi$ [10$^3$ (km/s)$^2$]"
)
...
ax2.text(
    0.04, 0.94, "z = 500pc", transform=ax2.transAxes,
    ha="left", va="top", fontsize=AXIS_LABEL_FONTSIZE,
...
ax3.text(
    0.04, 0.94, "z = 500pc", transform=ax3.transAxes,
    ha="left", va="top", fontsize=AXIS_LABEL_FONTSIZE,
    bbox=dict(facecolor="white", alpha=0.75, edgecolor="none")
)
cbar = fig.colorbar(
...
ax4.set_title(
    r"Spiral Contribution $\Phi_{\mathrm{spiral}}$", fontsize=TITLE_FONTSIZE
)
ax4.text(
    0.04, 0.94, "z = 500pc", transform=ax4.transAxes,
    ha="left", va="top", fontsize=AXIS_LABEL_FONTSIZE,
    bbox=dict(facecolor="white", alpha=0.75, edgecolor="none")
)
cbar = fig.colorbar(
