...
dir_import = (
    "D://Dropbox/Public Documents/UCLAN/AA3050 Dissertation/VOT files to use/"
)
file_import = "Pleaides Brandner 2023 20241020v1-result.vot.gz"


def Centre_of_Mass(t):
    arr_mass = np.array([t["mass_flame"]])
...
c1=SkyCoord(ra=qt["ra"], dec=qt["dec"], pm_ra_cosdec=qt["pmra"]*np.cos(2*np.pi*qt["dec"]/360), distance =qt["distance"], pm_dec=qt["pmdec"],radial_velocity=qt["radial_velocity"]) 
...
qt["mass_flame"] = qt["mass_flame"].astype(np.float64)
...
# compute cartesian co-ords and put in table
t["x"] = SkyCoord(ra=(qt["ra"]), dec=(qt["dec"]), distance=(qt["distance"]), frame="icrs").cartesian.x
t["y"] = SkyCoord(ra=(qt["ra"]), dec=(qt["dec"]), distance=(qt["distance"]), frame="icrs").cartesian.y
t["z"] = SkyCoord(ra=(qt["ra"]), dec=(qt["dec"]), distance=(qt["distance"]), frame="icrs").cartesian.z
...
t["mass_flame"]=t["mass_flame"].filled(np.average(t["mass_flame"]))   # fill in the blanks!
