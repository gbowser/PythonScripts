...
def compute_rotational_properties(positions, velocities, masses):
    ...
    :return: (avg_rotational_velocity, avg_tangential_velocity, avg_rotational_axis, total_mass, avg_mass)
    """
    ...
    # Compute total and average mass in the shell
    total_mass = np.sum(masses)
    avg_mass = np.mean(masses) if len(masses) > 0 else 0
    
    return avg_rotational_velocity, avg_tangential_velocity, avg_rotational_axis, total_mass, avg_mass

# Define file paths
base_path = "D://Dropbox/Public Documents/UCLAN/AA3050 Dissertation/Kinematics/2025-03-01"
...
for shell in range(1, max_radius + 1):
    ...
    if np.any(mask):  # Ensure there's data in this shell
        avg_rot_vel, avg_tan_vel, avg_rot_axis, total_mass, avg_mass = compute_rotational_properties(
