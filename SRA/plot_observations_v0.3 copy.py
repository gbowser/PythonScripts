...
def kinematics_maps(name_file, name_image, name_out, galaxy, PA_disc, inc_disc, PA_bar, vsyst, distance, \
                    VLIM, SMIN, SMAX, origin, slice_width, cutoff=None, fs=None, order=None, 
                    fig_maps=True, fig_profile=True, Rbar=1., bins1d=100, xlims=(-1.5, 1.5)):
    ...
    scale_pixel_to_kpc = 0.2 * (distance * 1e3) / 206265.
    ...
    if fig_maps == True:
        ...
        plt.suptitle(galaxy+' ,  i='+str(inc_disc)+r'$^\circ$ , $\Delta$PA='+str(np.round(DeltaPA))+r'$^\circ$ , slice width='+str(np.round(slice_width,2))+' kpc', fontsize=20)

        ax1 = fig_kin.add_subplot(221)
        plt.axline((0,0), slope=np.tan(np.radians(DeltaPA)), color='k', lw=3, ls=':', zorder=+20)
        plt.title(r' $V_{\rm LOS}$', fontsize=fs)
        c = ax1.pcolormesh(x_rot, y_rot, vlos, shading='auto', cmap='coolwarm', vmin=-VLIM, vmax=+VLIM)
        cbar = plt.colorbar(c, ax=ax1, label='[km/s]', shrink=SK)
        cbar.ax.tick_params(labelsize=16)
        cbar.ax.yaxis.label.set_size(18)
        contours = ax1.contour(x_rot, y_rot, image, levels=levels, colors='k', linewidths=LW)
        plt.fill_between([np.min(x_rot), np.max(x_rot)], -slice_width_pixel/2, +slice_width_pixel/2, color='k', alpha=0.5)
        ...
        c = ax2.pcolormesh(x_rot, y_rot, sigmalos, shading='auto', cmap='coolwarm', vmin=SMIN, vmax=SMAX)

        cbar = plt.colorbar(c, ax=ax2, label='[km/s]', shrink=SK)
        cbar.ax.tick_params(labelsize=16)
        cbar.ax.yaxis.label.set_size(18)
        contours = ax2.contour(x_rot, y_rot, image, levels=levels, colors='k', linewidths=LW)
        plt.fill_between([np.min(x_rot), np.max(x_rot)], -slice_width_pixel/2, +slice_width_pixel/2, color='k', alpha=0.5)
        plt.tick_params(labelsize=fs)
        plt.xlabel(' X (kpc)', fontsize=fs)
        plt.ylabel(' Y (kpc)', fontsize=fs)
        ...
        c = ax1.pcolormesh(x_rot, y_rot, err_vlos, shading='auto', cmap='coolwarm', vmin=0, vmax=20)
        cbar = plt.colorbar(c, ax=ax1, label='[km/s]', shrink=SK)
        cbar.ax.tick_params(labelsize=16)
        cbar.ax.yaxis.label.set_size(18)
        contours = ax1.contour(x_rot, y_rot, image, levels=levels, colors='k', linewidths=LW)
        ...
        c = ax2.pcolormesh(x_rot, y_rot, err_sigmalos, shading='auto', cmap='coolwarm', vmin=0, vmax=20)
        cbar = plt.colorbar(c, ax=ax2, label='[km/s]', shrink=SK)
        cbar.ax.tick_params(labelsize=16)
        cbar.ax.yaxis.label.set_size(18)
        contours = ax2.contour(x_rot, y_rot, image, levels=levels, colors='k', linewidths=LW)
    ...
    if fig_profile == True:
        ...
        F_bins16 = stats.binned_statistic(X, F,  statistic=lambda y: np.nanpercentile(y, 16), bins=bins1d)
        F_all16 = F_bins16.statistic.T
        F_bins84 = stats.binned_statistic(X, F,  statistic=lambda y: np.nanpercentile(y, 84), bins=bins1d)
        F_all84 = F_bins84.statistic.T

        
        # sc1 = ax1.scatter(bin_middles, F_all, c='yellow', s=ms, edgecolors ='k', zorder=+10, rasterized=True)
        sc1 = ax1.scatter(X[::strd], F[::strd], c='yellow', s=ms, edgecolors ='k', zorder=+10, rasterized=True)

        ax1.plot(bin_middles, F_all, c='k', lw=3., ls='-', zorder=20)
        ax1.plot(bin_middles, F_all, c='w', lw=1., ls='-', zorder=20)
        ...
        # Bar radius        
        ax1.axvline(Rbar/scale_pixel_to_kpc, c='k', ls='--', lw=2.)
        ax1.axvline(-Rbar/scale_pixel_to_kpc, c='k', ls='--', lw=2.)
        ...
        sc2 = ax2.scatter(X[::strd], S[::strd], c='cyan', s=ms, edgecolors ='k', zorder=+10, rasterized=True)
        
        S_bins = stats.binned_statistic(X, S, 'median', bins=bins1d)
        bin_middles = F_bins.bin_edges[:-1] + np.diff(F_bins.bin_edges)/2
        S_all = S_bins.statistic.T

        S_bins16 = stats.binned_statistic(X, S,  statistic=lambda y: np.nanpercentile(y, 16), bins=bins1d)
        S_all16 = S_bins16.statistic.T
        S_bins84 = stats.binned_statistic(X, S,  statistic=lambda y: np.nanpercentile(y, 84), bins=bins1d)
        ...
        ax2.plot(bin_middles, S_all, c='r', lw=2., ls='-', zorder=20)
        ...
        # Bar radius
        ax2.axvline(-Rbar/scale_pixel_to_kpc, c='k', ls='--', lw=2.)
        ax2.axvline(Rbar/scale_pixel_to_kpc, c='k', ls='--', lw=2.)
        ...
        ax3.plot(bin_middles, F_all, c='k', lw=3., ls='-', zorder=20)
        ax3.plot(bin_middles, F_all, c='w', lw=1., ls='-', zorder=20)
        ax3.fill_between(bin_middles, F_all16, F_all84, color='grey', alpha=0.3)


        # ax3t.plot(X, running_median_S, c='r', lw=2., ls='-', zorder=20)
        ax3t.plot(bin_middles, S_all, c='r', lw=2., ls='-', zorder=20)
        ...
        ax3.axvline(-Rbar/scale_pixel_to_kpc, c='k', ls='--', lw=2.)
        ...
        fig.savefig(os.path.join(output_folder, 'Obs_profiles_' + name_out + '.pdf'), format='pdf')
...
os.chdir('/Users/stuartanderson/Documents/Astronomy/Projects/MW_Bar_Orbits/Papers/Paper II/Obs_processing')

Rbar = 435 * (0.2 * (19.6 * 1e3) / 206265.)
xlims = (-1.5*Rbar, 1.5*Rbar)
kinematics_maps(name_file='NGC1300_MAPS_copt_0.89asec.fits',
                name_image= 'NGC1300_PHANGS_IMAGE_white_copt_0.89asec.fits',
                name_out='NGC1300', 
                galaxy='NGC 1300',
                PA_disc=278, inc_disc=32, PA_bar=99,
                vsyst=0, distance=19.6, VLIM=250, SMIN=0, SMAX=140, 
                origin=(745.,453.), slice_width=Rbar * 0.2, Rbar=Rbar, 
                fig_maps=False, fig_profile=True, bins1d=100, xlims=xlims)

# NGC 3627
# Bar radius is 23.75 arcsec
Rbar = 23.75 / 0.2 * (0.2 * (19.6 * 1e3) / 206265.)
xlims = (-1.5*Rbar, 1.5*Rbar)
kinematics_maps(name_file='NGC3627_MAPS_copt_1.05asec.fits',
                name_image= 'NGC3627_PHANGS_IMAGE_white_copt_1.05asec.fits',
                name_out='NGC3627', 
                galaxy='NGC 3627',
                PA_disc=173, inc_disc=57, PA_bar=160,
                vsyst=0, distance=11.3, VLIM=250, SMIN=50, SMAX=140, 
                origin=(336., 778.), slice_width=Rbar*0.2, Rbar=Rbar, 
                fig_maps=False, fig_profile=True, bins1d=100, xlims=xlims)


# NGC 1566
# Bar radius is 35 arcsec
Rbar = 35 / 0.2 * (0.2 * (19.6 * 1e3) / 206265.)
xlims = (-1.5*Rbar, 1.5*Rbar)
kinematics_maps(name_file='NGC1566_MAPS_copt_0.80asec.fits',
                name_image= 'NGC1566_PHANGS_IMAGE_white_copt_0.80asec.fits',
                name_out='NGC1566', 
                galaxy='NGC 1566',
                PA_disc=220, inc_disc=31, PA_bar=47.5,
                vsyst=0, distance=21.3, VLIM=250, SMIN=50, SMAX=140, 
                origin=(453., 447.), slice_width=Rbar*0.2, Rbar=Rbar, 
                fig_maps=False, fig_profile=True, bins1d=100, xlims=xlims)


# Bar radius is 95 arcsec NGC 1365
# Not a great one in terms of PA though so we have pollution from sigma_R
Rbar = 95 / 0.2 * (0.2 * (19.6 * 1e3) / 206265.)
xlims = (-1.5*Rbar, 1.5*Rbar)
kinematics_maps(name_file='NGC1365_MAPS_copt_1.15asec.fits',
                name_image= 'NGC1365_PHANGS_IMAGE_white_copt_1.15asec.fits',
                name_out='NGC1365', 
                galaxy='NGC 1365',
                PA_disc=220, inc_disc=40, PA_bar=130,
