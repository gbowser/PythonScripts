...
8. CALCUKATE PARAMETERS FOR SHOUKDEES FOR X<0 AND X>0
...
"""
def run_SRA(model, t_start, t_end, plot_derivs=True, generate_shoulders_file=False, 
...
#    smoothed_offset = 0
    ...
    if override_Data == None:
        ...
        output_folder = '/Users/stuartanderson/Documents/Astronomy/Projects/Density_Shoulder/Output'
    
    if override_folders != None:
        model_folder = override_folders[0]
        output_folder = override_folders[1]

    output_temp = '/Users/stuartanderson/Documents/Astronomy/Projects/Density_Shoulder/Output/Temp'
    ...
    if slice_actions == True:
        shoulders_fname = 'shoulders_roc_{0}_pctiles.npy'.format(Bin_by_action)
        descriptor = 'By action percentile' #This will be replaced in the code

    # Load the t=0 actions from their own file
    # Filename like D50_actions.npy
    Data_actions = None
    if slice_actions == True:

        if os.path.exists(os.path.join(model_folder, '{0}0_actions.npy'.format(model))):
            Data_actions = np.load(os.path.join(model_folder, '{0}0_actions.npy'.format(model)))
            Bin_by, Bin_by_labels = Data_actions[Bin_by_action], Bin_by_action

    timestamp = [str(x) for x in range(t_start, t_end + 1)]
    ...
    if override_bar_extent == None:
        bre_file = os.path.join(model_folder, '{0}_bar_radial_extent.npy'.format(model))
        if os.path.isfile(bre_file) == True:
            bre = np.load(bre_file)
        else:
            bre = []
    else:
        bre = []
...
#            bar_extent = max(bar_ends_phi2, bar_ends_a2)
        ...
        fname = '{0}{1}_stars.npy'.format(model, t)        
        print('\nProcessing time step {0} for model {1}'.format(t, model))
