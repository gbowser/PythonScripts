...
WORKFLOW_STEPS = [
    ...
    (
        ...
        r"s4g_image_downloader\geometry_output\s4g_image_geometry_manifest.csv",
    ),
    (
        "3",
...
def build_docx() -> None:
    ...
    add_body(
        ...
        "barprofiles_paper_GB_working_copy\\data\\scrambled_map.txt.",
    )
    add_bullets(
        doc,
        [
            r"Default input: barprofiles_paper_GB_working_copy\data\scrambled_map.txt.",
    ...
    add_bullets(
        ...
        [
            ...
            r"Default manifest output: s4g_image_downloader\geometry_output\s4g_image_geometry_manifest.csv.",
            r"Default cache folder: s4g_image_downloader\geometry_catalog_cache.",
    ...
    add_bullets(
        doc,
        [
            r"Default manifest: s4g_image_downloader\geometry_output\s4g_image_geometry_manifest.csv.",
...
def pdf_lines() -> list[str]:
    ...
    lines.extend(
        [
            ...
            "Key commands: python download_s4g_images.py --dry-run --limit 5; python download_s4g_images.py",
            "",
            "Program detail: build_s4g_geometry_manifest.py",
            r"Default output: s4g_image_downloader\geometry_output\s4g_image_geometry_manifest.csv",
            "Key commands: python build_s4g_geometry_manifest.py; python build_s4g_geometry_manifest.py --no-vizier",
            "",
            ...
            "Key commands: python plot_s4g_isophote_axes.py; python plot_s4g_isophote_axes.py --limit 3",
            "",
    ...
    with PdfPages(PDF_OUT) as pdf:
        ...
        for line in wrapped:
            ...
            if len(page_lines) >= 38:
                write_pdf_page(pdf, page_lines, page_number)
                page_number += 1
                page_lines = []
        if page_lines:
            write_pdf_page(pdf, page_lines, page_number)
