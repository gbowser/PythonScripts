...
def compute_rotational_properties(positions, velocities, masses):
    ...
    v_radial = (
        np.sum(velocities * r_hat, axis=1, keepdims=True) * r_hat
    ...
    valid_L = angular_momentum[
        L_magnitude > 1e-10
    ]  # Ignore zero angular momentum cases
    if len(valid_L) > 0:
        avg_rotational_axis = np.mean(
            valid_L / np.linalg.norm(valid_L, axis=1)[:, None], axis=0
        )
        avg_rotational_axis /= np.linalg.norm(
            avg_rotational_axis
