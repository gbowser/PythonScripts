...
fig_height, (ax_force_z, ax_frequency_z) = plt.subplots(2, 1, figsize=(10, 14))
fig_radius, (ax_force_radius, ax_frequency_radius) = plt.subplots(
    2, 1, figsize=(10, 14)
)
fig_angle, (ax_force_phi, ax_frequency_phi) = plt.subplots(2, 1, figsize=(10, 14))
summary_lines = [
...
for radius_kpc, color in zip(radii_kpc, colors):
    ...
    omega_curve = np.array(
        [
            ...
            for zmax_kpc in z_frequency_plot_kpc
        ]
    )
    ...
    force_phi_curve = np.array(
        [
            fz_physical_kms2_per_kpc(
                radius_kpc, angle_force_height_kpc, barphi_rad + phi_offset_rad
            )
    ...
    omega_phi_curve = np.array(
        [
            vertical_frequency_kmskpc(
                radius_kpc, angle_frequency_zmax_kpc, barphi_rad + phi_offset_rad
            )
            for phi_offset_rad in phi_scan_rad
        ]
    )
...
for z_sample_kpc, color in zip(radius_force_z_samples_kpc, radius_force_colors):
    ...
    ax_force_radius.scatter(
        [radius_force_reference_kpc], [reference_force], color=color, s=36, zorder=3
    )
...
ax_frequency_z.set_ylabel(
    r"$\Omega_z$ [km s$^{-1}$ kpc$^{-1}$]", fontsize=AXIS_LABEL_FONTSIZE
)
...
ax_force_phi.set_xlabel(
    r"Angle Relative to Bar [$^\circ$]", fontsize=AXIS_LABEL_FONTSIZE
)
...
ax_frequency_phi.set_xlabel(
    r"Angle Relative to Bar [$^\circ$]", fontsize=AXIS_LABEL_FONTSIZE
)
ax_frequency_phi.set_ylabel(
    r"$\Delta \Omega_z(\phi)$ [km s$^{-1}$ kpc$^{-1}$]", fontsize=AXIS_LABEL_FONTSIZE
)
...
fig_height.savefig(
    output_dir / f"{output_base}_height_{timestamp}.png", dpi=400, bbox_inches="tight"
)
fig_radius.savefig(
    output_dir / f"{output_base}_radius_{timestamp}.png", dpi=400, bbox_inches="tight"
)
fig_angle.savefig(
    output_dir / f"{output_base}_azimuth_{timestamp}.png", dpi=400, bbox_inches="tight"
)
