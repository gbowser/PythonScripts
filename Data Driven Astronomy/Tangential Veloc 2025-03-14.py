...
def compute_tangential_velocity_and_axis(input_file, output_file):
    ...
    v_dot_L = vx * rotational_axis[0] + vy * rotational_axis[1] + vz * rotational_axis[2]
    ...
    sign = np.sign(v_t_x * cross_product_x + v_t_y * cross_product_y + v_t_z * cross_product_z)
    ...
    for radius in bins:
        ...
        vt_mean = np.average(v_t_signed[mask], weights=m[mask])
