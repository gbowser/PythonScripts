...
$BatchScript = Join-Path $ForegroundDir "Batch tools\batch_sep_all_galaxies.py"
...
$SEPSpikeOut = Join-Path $SEPBatchParent "sep_spike_gate_20260719_183144"
$SEPToyOut = Join-Path $SEPBatchParent "sep_toy_object_20260722_141659"
...
Write-MasterLog "SEP all-galaxy batch runner started."

Run-Step `
    -Name "SEP Spike Gate all-galaxy batch" `
...
Run-Step `
    -Name "SEP Toy Object all-galaxy batch" `
...
if ($Failures -eq 0) {
    Write-MasterLog "SEP all-galaxy batch runner finished successfully."
    exit 0
}

Write-MasterLog "SEP all-galaxy batch runner finished with $Failures failed step(s)."
exit 1
