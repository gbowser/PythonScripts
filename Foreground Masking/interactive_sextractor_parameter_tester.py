#!/usr/bin/env python3
"""Interactive classic SExtractor foreground-mask parameter tester.

This wraps the Astromatic SExtractor command-line executable ("sex") while
keeping the same local S4G manifest/input/output and deprojected diagnostic
display style used by the other foreground-masking testers.
"""
...
DEFAULT_SEXTRACTOR_CMD = "sex"
DEFAULT_DETECT_THRESH = 3.0
DEFAULT_DETECT_MINAREA = 5
DEFAULT_DEBLEND_NTHRESH = 32
DEFAULT_DEBLEND_MINCONT = 0.005
DEFAULT_BACK_SIZE = 64
DEFAULT_FILTER_FWHM = 3.0
...
def prepare_detection_image(data: np.ndarray, mode: str) -> tuple[np.ndarray, np.ndarray]:
    if mode == "residual":
        residual = data - smooth_model(data, sigma_pixels=15.0)
        detection = np.array(residual, copy=True)
    else:
        residual = data - smooth_model(data, sigma_pixels=15.0)
        detection = np.array(data, copy=True)
    finite = np.isfinite(detection)
    if not np.any(finite):
        raise ValueError("Detection image contains no finite pixels.")
    detection[~finite] = float(np.nanmedian(detection[finite]))
    return detection, residual


def robust_sigma(data: np.ndarray) -> float:
    finite = np.asarray(data, dtype=float)
    sample = finite[np.isfinite(finite)]
    if sample.size == 0:
        return 1.0
...
def dilate_mask(mask: np.ndarray, radius_pixels: int) -> np.ndarray:
    ...
    return ndimage.binary_dilation(np.asarray(mask, dtype=bool), structure=circular_footprint(radius_pixels))


def split_command(command: str) -> list[str]:
    if not command.strip():
        raise ValueError("SExtractor command cannot be empty.")
...
def write_sextractor_files(work_dir: Path, params: dict[str, float | int | str]) -> tuple[Path, Path, Path]:
    config_path = work_dir / "default.sex"
    param_path = work_dir / "default.param"
    conv_path = work_dir / "gauss_3.0_5x5.conv"
    ...
    conv_path.write_text(
        "\n".join(
            [
                "CONV NORM",
                "# 5x5 Gaussian-ish convolution mask",
                "1  2  3  2 1",
                "2  5  8  5 2",
                "3  8 12  8 3",
                "2  5  8  5 2",
    ...
    config_path.write_text(
        "\n".join(
            [
                f"CATALOG_NAME     {work_dir / 'sextractor_catalog.cat'}",
                "CATALOG_TYPE     ASCII_HEAD",
                f"PARAMETERS_NAME  {param_path}",
                f"FILTER_NAME      {conv_path}",
                "FILTER           Y",
                f"DETECT_MINAREA   {int(params['detect_minarea'])}",
                f"DETECT_THRESH    {float(params['detect_thresh']):.6g}",
                f"ANALYSIS_THRESH  {float(params['detect_thresh']):.6g}",
                f"DEBLEND_NTHRESH  {int(params['deblend_nthresh'])}",
                f"DEBLEND_MINCONT  {float(params['deblend_mincont']):.6g}",
                "CLEAN            Y",
                "CLEAN_PARAM      1.0",
                "BACK_TYPE        AUTO",
                f"BACK_SIZE        {int(params['back_size'])}",
                "BACK_FILTERSIZE  3",
                "WEIGHT_TYPE      NONE",
                "CHECKIMAGE_TYPE  SEGMENTATION",
                f"CHECKIMAGE_NAME  {work_dir / 'segmentation.fits'}",
    ...
    return config_path, param_path, conv_path


def run_sextractor(input_fits: Path, work_dir: Path, params: dict[str, float | int | str]) -> np.ndarray:
    config_path, _param_path, _conv_path = write_sextractor_files(work_dir, params)
    command = split_command(str(params["sextractor_cmd"]))
    command.extend([str(input_fits), "-c", str(config_path)])
    try:
        subprocess.run(command, check=True, capture_output=True, text=True)
    except FileNotFoundError as exc:
        raise RuntimeError(
            "Classic SExtractor executable was not found. Install it, then set the command field "
            "to something like 'sex' or the full path to the executable."
        ) from exc
    ...
    segmentation_path = work_dir / "segmentation.fits"
    if not segmentation_path.exists():
        raise RuntimeError("SExtractor finished without writing segmentation.fits.")
    segmentation = np.squeeze(fits.getdata(segmentation_path))
    if segmentation.ndim != 2:
        raise ValueError(f"Expected 2D segmentation check-image, got {segmentation.shape}.")
    return np.asarray(segmentation)


def segment_rows(segmentation: np.ndarray, residual: np.ndarray, geometry: dict[str, float]) -> list[dict[str, float | int]]:
    labels = [int(label) for label in np.unique(segmentation) if label > 0]
    rows: list[dict[str, float | int]] = []
    sigma = robust_sigma(residual)
    median = float(np.nanmedian(residual[np.isfinite(residual)]))
    for label in labels:
        pixels = segmentation == label
        ...
        if area < 3:
            ...
            eig = np.maximum(np.linalg.eigvalsh(np.cov(coords, rowvar=False)), 0.0)
            elongation = float(np.sqrt(eig[-1]) / np.sqrt(eig[0])) if eig[0] > 0 else np.inf
        peak = float(np.nanmax(np.where(pixels, residual, np.nan)))
        distance = float(np.hypot(x_centroid - (geometry["xc"] - 1.0), y_centroid - (geometry["yc"] - 1.0)))
        rows.append(
            {
                "label": label,
                "area": area,
...
def sextractor_products(data: np.ndarray, header: fits.Header, params: dict[str, float | int | str], geometry: dict[str, float]):
    detection, residual = prepare_detection_image(data, str(params["detect_on"]))
    with tempfile.TemporaryDirectory(prefix="sextractor_") as temp:
        work_dir = Path(temp)
        input_fits = work_dir / "detection.fits"
        fits.PrimaryHDU(np.asarray(detection, dtype=np.float32), header=header).writeto(input_fits, overwrite=True)
        raw_segmentation = run_sextractor(input_fits, work_dir, params)
    rows = segment_rows(raw_segmentation, residual, geometry)
    filtered, updated_rows = filter_segmentation(raw_segmentation, rows, params)
...
class SExtractorTester(tk.Tk):
    def __init__(self, manifest: Path, pc_name: str):
        super().__init__()
        self.title("Classic SExtractor Parameter Tester")
        ...
        self.output_dir = remove_foreground_folder(pc_name) / "interactive_sextractor_parameter_tester"
    ...
    def _build_controls(self) -> None:
        ...
        ttk.Label(control, text="SExtractor command").pack(anchor=tk.W)
        self.sextractor_cmd = tk.StringVar(value=DEFAULT_SEXTRACTOR_CMD)
        ttk.Entry(control, textvariable=self.sextractor_cmd, width=34).pack(fill=tk.X, pady=(0, 8))
        ...
        self._scale(control, "detect_thresh", "DETECT_THRESH", 0.5, 10.0, 0.1, DEFAULT_DETECT_THRESH)
        self._spin(control, "detect_minarea", "DETECT_MINAREA", 1, 80, 1, DEFAULT_DETECT_MINAREA)
        self._spin(control, "deblend_nthresh", "DEBLEND_NTHRESH", 8, 64, 1, DEFAULT_DEBLEND_NTHRESH)
        self._scale(control, "deblend_mincont", "DEBLEND_MINCONT", 0.0001, 0.1, 0.0005, DEFAULT_DEBLEND_MINCONT)
        self._spin(control, "back_size", "BACK_SIZE", 16, 256, 8, DEFAULT_BACK_SIZE)
    ...
    def current_params(self) -> dict[str, float | int | str]:
        return {
            "sextractor_cmd": self.sextractor_cmd.get(),
            "detect_on": self.detect_on_var.get(),
            "detect_thresh": float(self.vars["detect_thresh"].get()),
            "detect_minarea": int(self.vars["detect_minarea"].get()),
            "deblend_nthresh": int(self.vars["deblend_nthresh"].get()),
            "deblend_mincont": float(self.vars["deblend_mincont"].get()),
    ...
    def reset_parameters(self) -> None:
        defaults = {
            ...
            "detect_minarea": DEFAULT_DETECT_MINAREA,
            "deblend_nthresh": DEFAULT_DEBLEND_NTHRESH,
            "deblend_mincont": DEFAULT_DEBLEND_MINCONT,
    ...
    def mark_needs_calculation(self) -> None:
        ...
        suffix = ""
        if not command_available(self.sextractor_cmd.get()):
            suffix = "\nSExtractor command is not currently found on PATH."
        if name:
            self.status.set(f"{self.pc_var.get()} | {name}: click Calculate to run SExtractor.{suffix}")

    def output_png_path(self, params: dict[str, float | int | str]) -> Path:
        self.output_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        stem = (
            f"{display.safe_filename(self.galaxy_var.get())}_sextractor_"
            f"thr{float(params['detect_thresh']):.1f}_"
            f"area{int(params['detect_minarea'])}_"
            f"deb{float(params['deblend_mincont']):.4f}_"
    ...
    def calculate_now(self) -> None:
        name = self.galaxy_var.get()
        try:
            self.status.set(f"Calculating {name} with classic SExtractor...")
            self._show_calculating_overlay()
            self.update_idletasks()
            data, header, geometry = self._load_galaxy(name)
            params = self.current_params()
            products = sextractor_products(data, header, params, geometry)
            ...
            self.status.set(
                f"{self.pc_var.get()} | {name} | SExtractor kept {kept} segments, "
            ...
            messagebox.showerror("SExtractor calculation failed", str(exc))
    ...
    def draw_products(self, name, original, products, params, geometry) -> None:
        ...
        self.ax_cleaned.set_title("SExtractor masked preview")
        ...
        self.ax_mask.set_title(f"Mask | thresh={float(params['detect_thresh']):.1f}, area={int(params['detect_minarea'])}")
        ...
        self.draw_isophote(self.ax_cleaned_isophote, cleaned_view, x_axis, y_axis, extent, "SExtractor processed isophotes", half_width, bar_sma)
        ...
        self.draw_profile(self.ax_cleaned_profile, cleaned_view, x_axis, y_axis, half_width, bar_sma, "SExtractor processed bar-major profile")
    ...
    def draw_parameter_box(self, params, products) -> None:
        ...
        text = (
            ...
            f"minarea={int(params['detect_minarea'])}   "
            f"deblend={int(params['deblend_nthresh'])}/{float(params['deblend_mincont']):.4f}   "
...
def main() -> None:
    args = parse_args()
    app = SExtractorTester(args.manifest, args.pc)
    app.mainloop()
