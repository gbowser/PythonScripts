...
DEFAULT_MOVE_FACTOR = 0.4402062884346303
DEFAULT_MIN_DISTANCE = 0.23669706301370264
DEFAULT_GAUSSIAN_FWHM = 2.083995043326491
...
DEFAULT_MAX_AREA = 2073
DEFAULT_MAX_ELONGATION = 15.48885527915943
...
class MTObjectsTester(tk.Tk):
    def __init__(self, manifest: Path, pc_name: str, mtobjects_root: Path | None):
        super().__init__()
        self.title("MTObjects Parameter Tester (No Spike Gate)")
        ...
        self.output_dir = remove_foreground_folder(pc_name) / "interactive_mtobjects_parameter_tester_no_spike_gate"
    ...
    def _build_controls(self) -> None:
        ...
        mto_frame.pack(fill=tk.X, pady=(4, 8))
        ttk.Label(
            mto_frame,
            text="↑/↓ shows which direction masks more pixels (more aggressive).",
            wraplength=300,
            justify=tk.LEFT,
        ).pack(anchor=tk.W, pady=(0, 6))
        self._spin(mto_frame, "alpha", "Significance alpha (↑ more aggressive)", 1.0e-8, 1.0e-3, 1.0e-6, DEFAULT_ALPHA)
        self._spin(mto_frame, "move_factor", "Move factor (↓ more aggressive)", 0.0, 1.0, 0.05, DEFAULT_MOVE_FACTOR)
        self._spin(
            mto_frame,
            "min_distance",
            "Minimum brightness distance (↓ more aggressive)",
            0.0,
            100.0,
            0.5,
            DEFAULT_MIN_DISTANCE,
        )
        self._spin(mto_frame, "gaussian_fwhm", "Gaussian FWHM [px] (smoothing scale)", 0.0, 8.0, 0.25, DEFAULT_GAUSSIAN_FWHM)
        ...
        self._spin(filter_frame, "minarea", "Minimum area [px] (↓ more aggressive)", 1, 80, 1, DEFAULT_MINAREA)
        self._spin(filter_frame, "dilation_radius", "Mask dilation [px] (↑ more aggressive)", 0, 12, 1, DEFAULT_DILATION_RADIUS)
        self._spin(filter_frame, "max_area", "Max segment area [px] (↑ more aggressive)", 10, 5000, 10, DEFAULT_MAX_AREA)
        self._scale(filter_frame, "max_elongation", "Max elongation (↑ more aggressive)", 1.0, 20.0, 0.25, DEFAULT_MAX_ELONGATION)
        self._scale(
            filter_frame,
            "exclude_center_pixels",
            "Central exclusion [px] (↓ more aggressive)",
            0.0,
            120.0,
            1.0,
            DEFAULT_EXCLUDE_CENTER_PIXELS,
        )
    ...
    def _build_figure(self) -> None:
        ...
        profile_grid = grid[1:, 1].subgridspec(2, 1)
