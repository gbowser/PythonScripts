...
runner="$project/Foreground Masking/Optimisation/run_final20_full_cross_validation.py"
...
watch_log="$output_root/full_cross_validation_watchdog.log"
stale_limit_seconds=1800

while [[ ! -f "$final_result" && ! -f "$rejected_result" ]]; do
    if pgrep -f "[r]un_final20_full_cross_validation.py" >/dev/null; then
        ...
        if [[ -n "$latest_update" ]]; then
            ...
            if (( age_seconds > stale_limit_seconds )); then
                ...
                pkill -TERM -f '[r]un_final20_full_cross_validation.py' || true
                sleep 30
            fi
        fi
        sleep 60
    ...
    printf '[%s] Optimisation process absent; resuming from saved folds/studies.\n' "$(date '+%F %T')" >> "$watch_log"
    "$python_bin" "$runner" \
        ...
        --study-storage-root "/root/final20-optuna-studies" \
        --workers 4 --initial-points 8 --max-iter 32 --toys-per-image 10 >> "$watch_log" 2>&1
    exit_code=$?
...
if [[ -f "$final_result" ]]; then
    status="Final MTObjects cross-validation result detected"
else
    status="MTObjects scientific rejection result detected"
fi
