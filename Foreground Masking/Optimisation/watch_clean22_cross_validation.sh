...
output_root="/mnt/d/Dropbox/Public Documents/UCLAN/MSc Research/Remove foreground objects/clean22_toy_optimisation"
...
while [[ ! -f "$final_result" && ! -f "$rejected_result" ]]; do
    ...
    "$python_bin" "$runner" \
        ...
        --study-storage-root "/root/clean22-optuna-studies" \
...
if [[ -f "$final_result" ]]; then status="Final MTObjects cross-validation result detected"; else status="MTObjects scientific rejection result detected"; fi
