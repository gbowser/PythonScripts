$base = "D:\Dropbox\Public Documents\UCLAN\MSc Research\Remove foreground objects\clean22_toy_optimisation"

while ($true) {
    Clear-Host
    "Revised clean-22, adaptive 10-toy cross-validation"
    ...
    foreach ($method in @("SEP_cross_validation", "MTObjects_cross_validation")) {
        ...
        if (!(Test-Path -LiteralPath $root)) { "Waiting to start..."; continue }
        ...
        "Held-out folds completed: $heldOut / 22"
        ...
        foreach ($fold in $folds) {
            ...
            $trials = if ($summary) { [Math]::Max(0, (Get-Content -LiteralPath $summary.FullName).Count - 1) } else { 0 }
        ...
        $bestName = if ($method -eq "SEP_cross_validation") { "sep_toy_cross_validation_best.json" } else { "mtobjects_toy_cross_validation_best.json" }
        if (Test-Path -LiteralPath (Join-Path $root $bestName)) { "FINAL RESULT COMPLETE" }
    }
