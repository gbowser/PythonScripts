#!/usr/bin/env python3
"""Run revised-clean-22 SEP then MTObjects leave-one-galaxy-out CV."""
...
def main() -> int:
    ...
    parser.add_argument("--study-storage-root", type=Path, default=Path("/root/clean22-optuna-studies"))
    ...
    common = [
        "--clean-list", str(args.clean_list), "--manifest", str(args.manifest),
        "--pc", "Desktop", "--injection-manifest", str(args.injection_manifest),
        ...
        "--max-iter", str(args.max_iter), "--toys-per-image", str(args.toys_per_image),
    ...
    run([sys.executable, str(SCRIPT_DIR / "cross_validate_toy_objects_SEP.py"), *common,
         "--output-dir", str(args.output_root / "SEP_cross_validation"),
         "--study-storage-dir", str(args.study_storage_root / "SEP")], "SEP 22-fold cross-validation")
    run([sys.executable, str(SCRIPT_DIR / "cross_validate_toy_objects_MTObjects.py"), *common,
         ...
         "--calibrate-bg-variance"], "MTObjects 22-fold cross-validation")
    print("\nAll revised-clean-22 cross-validation stages completed.", flush=True)
    return 0
